		...
		<div class="card mt-5 border-dark">
			...
			<h5 class="card-header text-center bg-dark text-white">
				<a href="${pageContext.request.contextPath}/seguros/afterlogin.jsp" class="text-decoration-none text-white"><i
					class="fas fa-arrow-left float-left"></i></a> ATUALIZAR
			</h5>
			<div class="card-body">
				<form class="p-3"
					action="${pageContext.request.contextPath}/seguros/clientServlet?acao=listarsegurados"
					method="post">
						<div class="form-group row">
						<label for="cpf"
							class=" col-sm-1 col-form-label col-form-label-sm">ID:</label>
						<div>
							<input class="form-control form-control-sm ml-5" id="id"
								name="id" placeholder="id"  readonly value="${client.id}">
						</div>
					</div>
					<div class="form-group row ">
						<label for="nome"
						...
						<div>
							<input class="form-control form-control-sm ml-5" id="nome"
								name="nome" placeholder="nome" required value="${client.nome}">
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div>
							<input class="form-control form-control-sm ml-5" id="cpf"
								name="cpf" placeholder="cpf" required value="${client.cpf}">
						</div>
					</div>
					<div class="form-group row">
						...
						<div>
							<input class="form-control form-control-sm ml-5" id="rg"
								placeholder="rg" name="rg" required value="${client.rg}">
						</div>
					</div>
					<div class="form-group row">
						...
						<div>
							<input type="date" class="form-control form-control-sm ml-5"
								id="aniversario" name="aniversario"  required value="${client.aniversario}">
						</div>
					</div>
					<hr>
					...
					<div class="form-check">
						<input class="form-check-input" type="radio" name="sexo"
							id="masculino" value="${client.sexo}"
							/><label
					...
					<div class="form-check">
						<input class="form-check-input" type="radio" name="sexo"
							id="feminino" value="${client.sexo} "
							${fn.marcacao == 'Feminino' ? 'checked' : ''}> <label
					...
					<div class="form-check">
						<c:forEach var="seguro" items="${seguro}">
							<input class="form-check-input" type="checkbox"
								...
								${fn.seguro.stream().anyMatch(v->v == '${seguro.id_seguro}').get() ? 'checked' : ''}>
					...
					<select class="custom-select mt-1" name="correntista">
						<option value="Sim"
							${fn.correntista == 'Sim' ? 'selected' : ''}>Sim</option>
						<option value="Nao"
							${fn.correntista == 'Nao' ? 'selected' : ''}>Não</option>
					</select>
					<h5 class="mt-2">DIA DA VISITA</h5>
					<select multiple class="custom-select mt-1" name="visitas">
						<option value="Segunda"
							${fn.visita.stream().anyMartch(v -> v == "Segunda").get() ? 'selected' : ''}>Segunda</option>
						<option value="Terça"
							${fn.visita.stream().anyMartch(v -> v == "Terça").get() ? 'selected' : ''}>Terça</option>
						<option value="Quarta"
							${fn.visita.stream().anyMartch(v -> v == "Quarta").get() ? 'selected' : ''}>Quarta</option>
						<option value="Quinta"
							${fn.visita.stream().anyMartch(v -> v == "Quinta").get() ? 'selected' : ''}>Quinta</option>
						<option value="Sexta"
